package pro.ms.simulador.infrastructure.MQTT;
...
import pro.ms.simulador.infrastructure.MQTT.consumer.MessagerRouterSuscriber;
import pro.ms.simulador.infrastructure.MQTT.publisher.MessageRouterPublish;
import pro.ms.simulador.infrastructure.MQTT.publisher.PublishTask;
...
public class MosquittoClient implements MQTTClient {
    ...
    @Override
    public void publish(TelemetryMessage<? extends Payload> telemetryMessage) {
        for(PublishTask task : messageRouterPublish.route(telemetryMessage)) {
            try {
                System.out.println("Enviando MQTT: " + task.topic() + " - " + task.messageToSerialize());
